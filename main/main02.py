...
# Import the resident data pipeline
from pipeline.resident_data import run_pipeline as run_resident_pipeline
...
class ExtendedApp(HitrotechApp):
    ...
    # ---- Resident Data Tool
    def action_resident_data(self):
        win = tb.Toplevel(self.root)
        win.title("Resident Data — Phone Numbers")
        win.geometry("560x420")
        ...
        def run_resident():
            ...
            def work():
                try:
                    ...
                    self.ui.show_info("✅ Done", "Resident Data pipeline finished for all files.")
                except Exception as e:
                    self.ui.show_error("❌ Error", f"Pipeline failed: {e}")

            self.run_with_loader(work)

        tb.Button(main_frame, text="Start List Building", bootstyle="success", command=run_resident, width=28).pack(pady=14)

    # ---- Extend handlers
    def get_action_handlers(self):
        handlers = super().get_action_handlers()
        handlers['vacant6'] = self.action_vacant_lot_6_phone
        handlers['resident'] = self.action_resident_data
        handlers['filesubtractor'] = self.action_file_subtractor
        return handlers


# -----------------------------
# Add tool definition for Vacant Lot, Resident Data and File Subtractor
# -----------------------------
TOOL_DEFS.append({
    'key': 'vacant6',
...
TOOL_DEFS.append({
    'key': 'resident',
    'title': 'Resident Data',
    'desc': 'Generate resident data lists with up to 6 phone numbers',
    'icon': '🏠',
    'action_text': 'Run',
})
